...
- ~~Stronger compositor/client isolation and permission model for GUI internals. (ledger RES-47; opener measured: gui_syscalls.c ALREADY gates 36 syscall cases behind require_owner/require_icon_owner — the series starts at a clipboard ACL, not at zero)~~ **Done (RESIDUE2 T7, 2026-09-06):** the permission model now has a second tier — per-window ops keep requiring ownership of THAT window, and GUI-GLOBAL state (kernel clipboard set+get, desktop theme set, taskbar notifications, desktop icons) requires `gui_pid_has_windows()` ("is a GUI participant at all"); theme READS stay open (no secrets in the struct).  Pinned by `test_gui_acl`: windowless → all denied, one window → all granted + clipboard round-trip, destroy → denied again (14 probes). (RESIDUE2 T7)
- ~~More complete text input, clipboard and focus behavior. (ledger RES-47)~~ **Done (RESIDUE2 T7, 2026-09-06):** measured-then-extended — textboxes already had caret+accent focus visuals, printable/backspace/Home/End/Del/arrows and Ctrl+C/Ctrl+V (against the now-ACL'd kernel clipboard); T7 adds Ctrl+X cut (copy+clear+change-event) in libauragui and shell-style command history in gterm (Up/Down recall ring, `history` builtin, duplicate-suppression, live-line parking). (RESIDUE2 T7)
- ~~Persisted user settings/theme. (ledger RES-47)~~ **Done (RESIDUE2 T7, 2026-09-06):** dotfile convention, no daemon — `ag_theme_save/save2/load` (31 named fields, versioning-by-omission, own hex parser because the libc mini-sscanf eats the 0x prefix), `/disk/.aura-theme` written fsync'd by gtheme, applied at desktop start by glaunch.  `test_gui_theme` proves it ACROSS A REBOOT (two boots, same disk image, cache=none): save 0xAA3311 → boot 2 `--show` returns it and glaunch reports loading it.  Bonus finds: gtheme's old save path was broken (O_RDONLY fd + write, never applied) and gtheme was never even in the initrd. (RESIDUE2 T7)
- [x] USB Manager GUI (`/gusb`) wired to `/usb` hotplug/storage status.
- ~~More GUI apps and richer file editor/terminal behavior. (RESIDUE2 T7)~~ **Done (RESIDUE2 T7, 2026-09-06):** gedit is a real multi-line editor (engine separated from chrome: insert/split/join/Delete, arrows/Home/End, POSIX trailing-newline load semantics, save+fsync; window has line numbers, caret, auto-scroll, Ln/Col+dirty status; `gedit --selftest` = 8 scripted engine checks incl. a /tmp round-trip); new app **gclip** (clipboard manager: set/paste/clear + live view, `gclip --selftest` round-trips the clipboard incl. short-buffer clamp, dogfooding the T7 ACL); gterm gained command history.  Gates: `test_gui_apps` 9/9; README rows added (gclip, gtheme; gedit enriched). (RESIDUE2 T7)
...
- ~~CI screenshots artifact. (class: CI)~~ **Done (RESIDUE2 T9, 2026-09-06):** the CI image now installs vncdotool + python3-pil (the GUI cases' VNC captures used to soft-skip in CI -- there were no screenshots to upload), and the gui shard gained an ALWAYS-ON `gui-screenshots` artifact step (`build/integration-logs/**/gui-screenshots/*.png`, kept on green -- visual evidence is wanted precisely when the shard passes, the failure dump already carried it).  Pipeline proven locally end-to-end: 01_desktop.png + 02_with_launcher.png (720x400 PNG) land exactly on the artifact path. (RESIDUE2 T9)
- ~~Add GDB helper scripts / pretty-printers for kernel structures. (RESIDUE2 T9)~~ **Done (RESIDUE2 T9, 2026-09-06):** `tools/gdb/` -- pretty.py (signature-matched printers: gui_theme_t as #RRGGBB, struct tcb with state mnemonics, struct ofd; falls through to raw display on no-match, registers portably without the gdb.printing module Debian omits) + aura.gdb (aura_theme / aura_default_theme / aura_windows commands) + README.  Gate `test_gdb_scripts.sh` (in make test-unit; skips loudly without gdb/kernel.elf; CI installs gdb): module compiles, kit loads against build/kernel.elf's DWARF, the const default theme renders THROUGH the printer (#2F60C0), struct tcb resolves, commands degrade cleanly targetless. (RESIDUE2 T9)
- ~~Reduce integration-test timing flakiness around process spawn and serial input pacing. (RESIDUE2 T9)~~ **Done (RESIDUE2 T9, 2026-09-06):** the sleep-based serial driver is now CONSUMPTION-gated: `_il_feed_queue` polls the live serial log and only types the next line after a NEW `auralite#` prompt appears (the previous command finished and the shell re-entered its read loop); the first line waits for the shell's FIRST prompt (up to 90s: TCG through the full selftest), retiring the guess-the-boot-time race class; 15s/line cap degrades GUI-app/`exit` cases to exactly the legacy behaviour; IL_FEED_SYNC=0 restores the old driver.  An off-by-one in the first cut (the boot prompt consumed the first line's authorisation -- every case lost its first command + 15s) was caught by the regression run and fixed with a decrement.  Receipt: the spawn-heavy set green 7/7 (spawn_argv_hostile x3, process_cleanup, execve_args, shell_commands, gui_apps), and the gated feed is FASTER than the blind sleeps (execve_args 28s, shell_commands 44s). (RESIDUE2 T9)
