...
## Status: COMPLETE ✅ — W32-0 – W32-8 done
...
| W32-8 Integration and documentation | ✅ done |
...
### Phase W32-8 — Integration, documentation and the honest matrix ✅ DONE

**Objective:** make it usable and make its limits legible.

#### Tasks

- [x] `docs/win32.md`: a generated function table, behaviour notes, and a
      blunt statement of what will not run.
- [x] `docs/status.md` gains a `w32` row.
- [x] `w32/examples/`: console, GUI, and a deliberately unsupported binary.
- [x] `make w32-sdk` and `make w32-sdk-check`, mirroring `make sdk`.
- [x] Shell: `run app.exe` detects PE by magic and routes it.
- [x] `README.md` gains a `w32` entry with the §1.3 disclaimer.

#### The bug this phase existed to find

Every earlier phase tested a mechanism with a hand-written fixture. That was
the right way to isolate faults, but it left one question open, and W32-8
answered it badly at first: **`run k32test.exe` did not work.**

The kernel recognises PE by magic and loads it (W32-3), so the binary
started — and then took a page fault on its first call, because nothing had
bound its import table. Import resolution lives in user space by design (D2),
so the direct kernel exec path never performs it. W32-3's fixtures have no
imports, which is exactly why the gap survived five phases of green tests.

The fix is the routing this phase was going to add anyway, but now for a
reason found rather than assumed: the shell reads the headers, and a PE with
a non-empty import directory goes through `/apps/w32run`. A PE **without**
imports is still spawned directly, because the kernel path applies
per-section W^X and is the hardened one — routing everything through
`w32run` would have quietly traded that away for uniformity.

The first version of the check also gated on `!argv`, which meant it never
fired: `run x.exe` always supplies an argv. Testing the *file* rather than
the caller's arguments is the whole point.
...
**Result:** `test_w32_integration` 12/12 and `make w32-sdk-check` 12/12.

The headline is one line of serial output:

```
[shell] /tests/w32hello.exe imports Win32 DLLs; running via /apps/w32run
w32run: /tests/w32hello.exe mapped at 0x400000000000, 7 import(s) bound
Hello from a Win32 console program on AuraLite OS.
command line: /tests/w32hello.exe
HeapAlloc worked.
```

That is **mingw-w64 output**, not a hand-written fixture — real code
generation, real import thunks, a real section layout — running unmodified.

The unsupported example is as much a deliverable as the working ones. It
imports `ADVAPI32` (the registry, a non-goal under D8), and the gate asserts
both that the refusal names the import *and* that the program's first
`WriteFile` never printed: the failure happens at load time, before a single
instruction runs. A loader that bound what it could and zeroed the rest would
run it until it happened to touch the registry and then crash with no
explanation.

The function table is generated by `tools/gen_w32_api_table.py`, checked in
`make test-unit`, and the check was verified by adding a phantom export and
confirming it fails. 44 functions across three modules.

`make w32-sdk` deliberately ships **no headers**: a Win32 program uses
mingw-w64's `<windows.h>`, which is the entire point of a personality, and
vendoring a copy would duplicate something that already exists and would go
stale. Only the import libraries, the examples and the documentation are
staged.

**Deliverable:** `docs/win32.md` (generated table),
`tools/gen_w32_api_table.py`, `tools/mkw32sdk.sh`, `tools/w32_sdk_check.sh`,
`w32/examples/{console-app,gui-app,unsupported-app}/`, PE routing in
`userspace/system/init/init.c`, `make w32-sdk` / `w32-sdk-check`,
`tests/integration/cases/test_w32_integration.sh`, `docs/status.md` and
`README.md` entries ✅
