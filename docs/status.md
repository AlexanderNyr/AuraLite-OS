...
| LAPIC / IOAPIC | ✅ | LAPIC enable + per-CPU timer and I/O APIC bring-up are implemented (MATURITY M2): the BSP runs on APIC IRQs (PIT@GSI2, kbd@GSI1), the IOAPIC base is cross-checked against the ACPI MADT at boot (`[ioapic] base … (MADT agree)`, RESIDUE R11), and legacy PIC/PIT paths remain available. MSI/MSI-X and MADT Interrupt Source Overrides remain named residue (RES-36 / ioapic.c). |
...
| **FAT32 — full** | ✅ | `/fat`: subdirs, **LFN (UCS-2 read+write)**, mkdir/rmdir/unlink/rename/truncate, FSInfo, FAT date/time stamps decoded through `stat()`. Since OTA O1 the mount is **partition-aware**: GPT entries first, then non-protective MBR entries, then the table-less raw-LBA-64 scratch contract (`[fat32] found FAT32 partition at LBA 256 (via GPT)` on the dual image); a table-bearing disk with no FAT32 volume is never formatted, and neither are diskfs (LBA 2) nor the AHCI self-test (LBA 1) allowed to touch a partitioned disk's sectors. |
...
| **Btrfs** | 🧪 | `/btrfs`: CoW B-tree filesystem with per-block SHA-256 checksums (32-byte trailer verified on every read; RESIDUE2 T3, kernel-local `kernel/lib/sha256.c`), extent tree, directory/inode items, full mutation surface. **Experimental — the gaps are deliberate**: subvolumes and snapshots are NOT implemented (single FS tree only); no send/receive, compression or RAID. |
...
| Power / reboot | ✅ | `SYS_REBOOT` (612, OTA O2): `fs_cache_sync` then the 8042 reset pulse, receipts `[reboot] SYS_REBOOT: flushing filesystems` / `[reboot] pulsing 8042 reset`; an ignored pulse halts loudly. Under QEMU without `-no-reboot` one serial log carries both boots — the OTA second-boot leg. |
...
| Feature | Status | Notes |
|---|---:|---|
| **Tooling and close-out (RESIDUE2 T9)** | ✅ | Consumption-gated serial input for the integration driver (each line waits for the next shell prompt — spawn-heavy set 7/7 and faster), tools/gdb pretty-printer kit with its own unit gate, CI screenshots artifact (vncdotool+PIL in the image, always-on upload). RESIDUE2_PLAN complete: TODO 40→7, ledger OPEN 6→1, phases 10/10, zero-warning builds. |
| **Ports and oddities (RESIDUE2 T8)** | ✅ | RES-02 root-caused and FIXED: SMAP made the boot shell's raw CPL0 frame writes fault-loop, so the shell never entered Ring 3 under `-cpu max` (banner/prompt/uname now land — cpumax smoke 8/8). RES-06: `arch_irq_enable()` on all four irqflags backends, vfs.c's pipe-wait `sti` migrated, the fd/OFD/pipe file compiles at all four widths (width lane 2b). RES-18: PIE on the i386 tenant — elf32load takes ET_DYN, relocates R_386_RELATIVE at base 0x10000000, `/bin32/pie32` runs and proves it (`i386_pie_smoke.sh` 6/6). |
| **GUI beyond the demo (RESIDUE2 T7)** | ✅ | Two-tier GUI permission model: per-window ops require owning that window, GUI-global state (clipboard set/get, theme set, notifications, icons) requires owning any window at all (`gui_pid_has_windows`, `test_gui_acl` 14 probes). Theme persists as a dotfile (`/disk/.aura-theme`) applied by the desktop at start — proven across a real reboot (`test_gui_theme`). gedit is a multi-line editor with a headless engine gate, gterm has command history, new /apps/gclip manages the ACL'd clipboard (`test_gui_apps`). |
| **Modern NIC data paths (RESIDUE2 T6)** | ✅ | vmxnet3 (VMware paravirtual rev-1: shared driver/queue descriptors, generation-bit rings, INTx autoMask — `drivers/vmxnet3/`) and e1000e (Intel 82574L legacy-descriptor path, MAC from reset-loaded RAL/RAH — `drivers/e1000e/`) are full netdev backends with the T5 contract (idle drain, blocking recv): DHCP DORA → ARP → ICMP ping → DNS resolve proven through each over SLIRP (`test_vmxnet3`, `test_e1000e`, 14 asserts each).  Netdev priority: e1000 → virtio-net → vmxnet3 → e1000e → rtl8139.  Writable FAT32 on USB (in-place + slack-grow, host-verified persistence) and the ext2 hotplug automount landed in the same phase — see the USB docs. |
| **Blocking edges + real web (RESIDUE2 T5)** | ✅ | Four boxes closed, each gated. (1) Idle RX drain: the passive poller drains e1000/rtl8139/virtio rings when no consumer exists — no more RX-overrun spam (`test_idle_rx_drain`, L2 lab). (2) UDP `recvfrom` fully blocking: parks on the NIC RX wait queue via `NET_WAIT_FOREVER`; link-down maps to `-ENETDOWN` (was raw -1 == EPERM) (`test_udp_blocking`, L2 lab: knock → delayed reply → blocked past the host delay). (3) Production TCP box audited STALE — sliding windows, Reno CC, retransmit queue + SACK, RFC 6298 RTO, PMTUD, OOO receive already lived in tcp.c (X5/Y1/M6); the real residue was content integrity: `test_tcp_x5` (1 MiB piecemeal-ACK upload; pass dated 2026-09-05) and `test_tcp_ordering` (NEW: 512 KiB position-dependent pattern verified BOTH directions through a host echo server, ~3 MB/s over SLIRP — any reorder/duplicate/corruption fails at a named offset). (4) TLS on the real internet: `test_realweb_rustlang` fetches https://rust-lang.org/ from the guest through SLIRP onto the runner's real network — DNS A+AAAA, v6 attempted/v4 fallback, TCP:443, TLS 1.3 `X25519MLKEM768`, chain validated against the 17 shipped roots, HTTP 200 with an 18594-byte body, clean FIN (dated 2026-09-05). |
...
| e1000 TX/RX | 🧪 | Legacy descriptor rings with INTx IRQ enable, IRQ cause handling, preallocated software RX queue, non-blocking compatibility receive, and blocking/timed receive helpers. TCP, ARP, DHCP, ICMP, and kernel UDP/DNS receive waits now use IRQ/wait-queue-backed bounded NIC waits; user UDP sockets (`sendto`/`recvfrom`) and the full production TCP path (sliding window, Reno CC, SACK retx queue, adaptive RTO — see the TCP client row) are implemented; idle rings are drained by the passive poller and UDP `recvfrom` parks on the RX wait queue (RESIDUE2 T5). |
...
| xHCI controller | 🧪 | **Real.** Event ring and command ring (U1), Enable Slot / Address Device with `Slot State` read back from the device context (U3), control transfers with short-packet residue and stall recovery (U4), bulk (U5), interrupt endpoints (U6), nested hubs with correct route strings (U9). The interrupt is taken and acknowledged, but the event ring is still drained by the polling consumer, so hotplug latency is the poll's (~400 ms), not interrupt time — see [`docs/usb.md`](usb.md) and [`USB_PLAN.md`](plans/USB_PLAN.md) U8. Streams/UAS absent. |
...
| `/ota` | ✅ | OTA update tool (OTA_PLAN O0–O5 complete): `check`/`apply`/`rollback`/`status`. Streams the payload over its own minimal HTTP GET in 4 KiB chunks straight into `/fat/KERNEL.NEW` (sha256 on the fly via libatls — the ~2.9 MiB kernel is never buffered whole), verifies against the manifest, then renames the A/B slots (`KERNEL.ELF`↔`KERNEL.OLD`) and `sync()`s. Exit gate: `test_ota_apply.sh` boots 0.0.1, installs a `0.0.2-ota` kernel over the network, reboots into it, and rolls back — three boots, one serial log. |
...
- **SMP scheduling (corrected by the residue ledger's R5 pass — this row was stale twice over).**  Per-CPU run queues, least-loaded placement and work stealing have been live since SMP 3.2, and user threads DO run on APs: the scheduler prints a one-time receipt (`[sched] R5 receipt: user thread pid=N on AP cpu=M`) and `test_fpu_smp` pins it.  The former named gap — a DEVICE interrupt waking a hlt-ed AP (RES-16) — closed at RESIDUE2 T2: receipt `[smpwake] PASS: 9 RTC(GSI8) device IRQs delivered to cpu1 … hlt looper woken 8 times`, pinned by `tests/integration/cases/test_irq_ap_wake.sh`.
- **TCP is small but not minimal.** 16 concurrent connections with a sliding send window (min(cwnd, peer window)), RFC 6928 initial window, RFC 6298-style adaptive RTO, dup-ACK classification with fast retransmit/recovery and the RFC 5681 §3.2 ssthresh arithmetic, SACK, Nagle, delayed ACK, TIME_WAIT, listen backlog, SO_REUSEADDR and RFC 1122 keepalive (MATURITY M6, REALINTERNET X5, RINET2 Y1–Y2).  What it still lacks versus production stacks: selective window scaling, ECN, and a per-route congestion state; fixed ISNs per handle remain a documented simplification.
- **Advanced filesystems are experimental but complete-surfaced.** Since FSFULL F2–F7, `ext4`, `btrfs`, `f2fs`, `exFAT` and `NTFS` sit on the shared buffer-cache block path and carry full (or, for NTFS, deliberately read-only) mutation surfaces with external interop gates (`mkfs.ext4`/`fsck.ext4`, `fsck.exfat`, `mkntfs`, in-kernel structural fscks) registered in the `fsfull` CI shard.  They remain hobby-OS implementations: no foreign JBD2 recovery, no btrfs subvolumes/snapshots, no f2fs GC, ASCII-only NTFS transcoding — the deliberate gaps are listed per-driver in the matrix above.
