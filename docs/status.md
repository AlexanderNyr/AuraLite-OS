...
| **OpenGL (libgl)** | 🚧 | User-space GL 1.1 stack, software rasterizer — see `GL_PLAN.md`. **G0-G2 complete**: hardened `GUI_OP_BLIT` presentation path, libc float math, `glmath` layer, the AuraGLX context, and now matrix stacks + immediate mode (`glBegin`/`glVertex`/`glEnd`, all ten primitive modes, `glRotatef`/`glFrustum`/`glOrtho`, Bresenham lines with colour interpolation). 124 host unit tests + 29 QEMU assertions. `/glcube` renders a rotating wireframe cube. Triangles are outlines only — the filled, depth-tested rasterizer lands in G3. |
