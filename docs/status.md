...
| **OpenGL (libgl)** | 🚧 | User-space GL 1.1 stack, software rasterizer — see `GL_PLAN.md`. **G0-G3 complete**: hardened `GUI_OP_BLIT` presentation path, libc float math, `glmath` layer, AuraGLX context, matrix stacks + immediate mode (all ten primitive modes), and a **filled edge-function rasterizer** with depth buffer (all 8 compare functions, `glDepthMask`), back-face culling, top-left fill rule, scissor test and `glPolygonMode`. 168 host unit tests + 38 QEMU assertions. `/glcube` renders a **solid depth-buffered cube**. Still missing: frustum clipping (G4 — geometry crossing the near plane is dropped, not split), lighting (G5), textures (G6), vertex arrays (G7). |
