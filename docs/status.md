...
| App | Status | Notes |
|---|---:|---|
| `/init` shell | ✅ | Interactive serial/keyboard shell; 34 builtins, job control (`jobs`/`fg`/`bg`), exit statuses and script execution (`sh <file> [args]` with `$0..$9`/`$#`/`$?`) since SELFHOST SH6a; redirects (`>`/`>>`/`<`), shell variables (`set`/`unset`/`$NAME`) and quote-aware parsing since SH6b; pipes (`|`) and command lists (`;` `&&` `||`) since SH6c. |
