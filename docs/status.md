...
| **OpenGL (libgl)** | 🚧 | User-space GL 1.1 stack, software rasterizer — see `GL_PLAN.md`. **G0+G1 complete**: hardened `GUI_OP_BLIT` presentation path, libc float math, `glmath` vector/matrix layer, and the AuraGLX context (`aglxCreateContext`/`MakeCurrent`/`SwapBuffers`/`Resize`) with `glClear`, `glViewport`, `glGetError`, `glGetString`. 74 host unit tests + 20 QEMU assertions. An application can create a context, clear it and present a frame. No geometry yet: immediate mode lands in G2, the triangle rasterizer in G3. |
