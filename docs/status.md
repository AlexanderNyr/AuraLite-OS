...
| **Self-hosting toolchain** | 🧪 | SELFHOST_PLAN.md SH1+SH2+SH3: guest TinyCC (`make selfhost-deps selfhost-tcc`) compiles, links and runs AuraLite's own userland in-guest (`/bin/tcc`, sources under `/src` in the initrd; `test_selfhost_tcc.sh` + `test_selfhost_userland.sh` in the `selfhost` shard), and SH3's `aulink` (`tools/aulink/aulink.c`) links ELF in-guest without `ld.lld` (`test_aulink.sh` host parity). `SPAWN_MAX_IMAGE` 16 MiB, user stack 4 MiB, tmpfs 256 files/volume, malloc 16-aligned, shell MAX_ARGS 32 / INPUT_MAX 512 — all raised/fixed for the compiler workload. SH4 (the in-guest assembler) is split into SH4a–SH4e along the output-format gradient; SH4a+SH4b+SH4c landed — `tools/mini-asm/mini-asm.c` assembles all four `-f bin` boot files (both MBRs, the SMP trampoline, and the 5632-byte `stage2_start.asm` with its 13-file include chain) byte-identically to nasm: full SIB/segment-override/0x67 effective-address model, 16/32/64-bit modes, `%include`/`%if`/`%define` preprocessor (`tests/unit/test_asm_parity.sh`, `[selfhost] asm PASS (bin): 4/4`); SH4d (elf64)–SH4e (elf32 + in-guest run) pending. |
