...
| **OpenGL (libgl)** | 🚧 | User-space GL 1.1 stack, software rasterizer — see `GL_PLAN.md`. **G0 complete**: `GUI_OP_BLIT` presentation path implemented and hardened, libc float math, `glmath` vector/matrix layer (37 host unit tests). No drawing API yet: context/`glClear` land in G1, immediate mode in G2, the triangle rasterizer in G3. |
