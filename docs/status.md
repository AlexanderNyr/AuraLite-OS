...
| **Self-hosting toolchain** | 🧪 | SELFHOST_PLAN.md SH1+SH2+SH3: guest TinyCC (`make selfhost-deps selfhost-tcc`) compiles, links and runs AuraLite's own userland in-guest (`/bin/tcc`, sources under `/src` in the initrd; `test_selfhost_tcc.sh` + `test_selfhost_userland.sh` in the `selfhost` shard), and SH3's `aulink` (`tools/aulink/aulink.c`) links ELF in-guest without `ld.lld` (`test_aulink.sh` host parity). `SPAWN_MAX_IMAGE` 16 MiB, user stack 4 MiB, tmpfs 256 files/volume, malloc 16-aligned, shell MAX_ARGS 32 / INPUT_MAX 512 — all raised/fixed for the compiler workload. SH4 (the in-guest assembler) is split into SH4a–SH4e along the output-format gradient; SH4a+SH4b landed — `tools/mini-asm/mini-asm.c` assembles both MBR variants and the SMP trampoline byte-identically to nasm, 64-bit mode/REX/control-regs included (`tests/unit/test_asm_parity.sh`, `[selfhost] asm PASS (bin): 3/4`); SH4c (stage2)–SH4e pending. |
