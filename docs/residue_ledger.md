...
| RES-41 | S | HANDED-OFF@R12 | TGSI backend for VirGL DRAW_VBO (GL G13) | OPENER, measured: the KERNEL side already emits the 12-dword DRAW_VBO packet (virgl.c:180); glvirgl.c:13 names its own gap — "does NOT implement DRAW_VBO … needs shaders" — the missing piece is a userspace TGSI assembler feeding CREATE_SHADER, nothing kernel-side |
