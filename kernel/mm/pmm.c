...
/* Reserve the low 56 MiB for boot-critical data the loader placed there.
 *
 ...
 * on another host) overflowed it and failed `make iso` in CI.  The raise
 * to 40 MiB bought a 16 MiB slot -- and the self-hosting closure
 * (/bin/tcc + the /src tree, SELFHOST_PLAN SH8) then grew the archive to
 * ~19.9 MiB, overflowing THAT in turn.  56 MiB gives the archive a 32 MiB
 * slot: ~1.6x the closure-staged size, and big enough that the next
 * growth is a plan question, not a surprise red CI.  The reserve costs
 * ~11% of the smallest configuration we boot (512 MiB) -- the same order
 * as the 40 MiB it replaces.  Everything else living in the reserve (the
 * SMP trampoline at 0x7000/0x8000, the kernel image at 1 MiB, its staging
 * buffer at 2 MiB, and the boot page tables at 16 MiB) sits below 24 MiB
 * and is unaffected.  Keep in step with: INITRD_MAX_BYTES in
 * boot/bios/stage2/stage2_start.asm, EARLY_RESERVE in
 * kernel/arch/i386/pmm32.c, and the mkiso bounds (tools/selfhost/mkiso.c,
 * tools/mkisoimage_dual.sh). */
#define PMM_EARLY_BOOT_RESERVE (56ULL * MIB)
