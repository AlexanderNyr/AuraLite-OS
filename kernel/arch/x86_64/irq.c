...
void pic_disable_for_apic(void) {
    /* Mask every IRQ on both PICs so they emit nothing, then flip the IMCR to
     * APIC delivery (8259 INTR fully decoupled from the BSP).  This is the
     * complement of pic_init()'s early-boot IMCR=PIC forcing, run once the
     * I/O APIC has taken over the legacy IRQs. */
    outb(PIC1_DATA, 0xFF);
    outb(PIC2_DATA, 0xFF);
    outb(IMCR_ADDR_PORT, IMCR_SELECT);
    outb(IMCR_DATA_PORT, IMCR_MODE_APIC);
}
...
void irq_dispatch(int irq, struct registers *regs) {
     ...
     * and the interrupt controller must be free to deliver the next tick when
     * we eventually return.  For edge-triggered IRQs (like the PIT) early EOI
     * is safe.  Once ioapic_init() has switched the BSP to I/O-APIC delivery
     * the 8259 is masked/decoupled, so only the Local APIC EOI below is
     * needed -- pic_eoi() would poke a disconnected, all-masked PIC. */
    if (!apic_irq_mode) {
        pic_eoi(irq);
    }
    /* Also signal EOI to the LOCAL APIC.  Two delivery paths share this
     * dispatcher:
     *   - BSP legacy IRQs arrive either via LINT0 in ExtINT mode (PIC path)
     *     or -- after ioapic_init() -- straight from the I/O APIC; in the
     *     ExtINT case the 8259's vector came from the INTA bus cycle and
     *     never entered the LAPIC's in-service register, so writing EOI is a
     *     harmless no-op, while in the I/O-APIC case it IS required.
     *   - Each AP's own LAPIC timer (vector 32 == IRQ 0's slot, started by
     *     ap_entry() in smp.c) always sets the in-service bit, and withholding
