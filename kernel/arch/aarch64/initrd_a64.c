/* kernel/arch/aarch64/initrd_a64.c -- USTAR initrd reader (ARM64_PLAN A5c; initrd_rv.c mechanically ported).
 *
 * initrd_rv.c's parsing rules at the fourth ISA: 512-byte headers,
 ...
 * pmm_a64_init marked those frames used, so the data pointers handed
...
int initrd_a64_init(const boot_info_t *bi)
{
    if (!bi->initrd_phys || !bi->initrd_size) {
        pl011_puts("[initrd] none present (-initrd not passed)\n");
        return -1;
    }

    archive      = (const uint8_t *)p2v_a64(bi->initrd_phys);
    ...
    if (!(h->magic[0] == 'u' && h->magic[1] == 's' && h->magic[2] == 't' &&
        ...
        pl011_puts("[initrd] bad ustar magic -- refusing the archive\n");
        archive = 0;
        return -1;
    }
    ...
    pl011_puts("[initrd] USTAR archive: ");
    put_udec_(file_count);
    pl011_puts(" files, ");
    put_udec_(archive_size);
    pl011_puts(" bytes (the one four-tenant archive)\n");
    return 0;
}
