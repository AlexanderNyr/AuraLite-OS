/* atls_sha512.c — SHA-512 (FIPS 180-4), streaming + one-shot.
 *
 * Required by Ed25519 (RFC 8032).  Same Merkle-Damgard shape as
 * atls_sha256.c with 64-bit words and 128-byte blocks.
 */
...
static void sha512_block(atls_sha512_ctx *c, const uint8_t p[128]) {
    uint64_t w[80];
...
void atls_sha512_update(atls_sha512_ctx *c, const void *data, size_t len) {
    ...
    uint64_t old = c->nbytes_lo;
    c->nbytes_lo += len;
    if (c->nbytes_lo < old) c->nbytes_hi++;
    while (len > 0) {
...
void atls_sha512_final(atls_sha512_ctx *c, uint8_t out[64]) {
    uint64_t bitlo = c->nbytes_lo << 3;
    uint64_t bithi = (c->nbytes_hi << 3) | (c->nbytes_lo >> 61);
    uint8_t pad = 0x80;
    atls_sha512_update(c, &pad, 1);
    uint8_t zero = 0;
    while (c->buflen != 112) atls_sha512_update(c, &zero, 1);
    uint8_t lenb[16];
    be64_put(lenb, bithi);
    be64_put(lenb + 8, bitlo);
    atls_sha512_update(c, lenb, 16);
    for (int i = 0; i < 8; i++) be64_put(out + i * 8, c->h[i]);
...
void atls_sha512(const void *data, size_t len, uint8_t out[64]) {
    atls_sha512_ctx c;
    atls_sha512_init(&c);
    atls_sha512_update(&c, data, len);
    atls_sha512_final(&c, out);
}
