...
# exiting process.  Runs /tests/proctest, which opens one window then returns 0;
...
il_assert_grep "$LOG" "\\[thread\\] '/tests/proctest' \\(tid [0-9]+\\) exited" "thread_exit ran for proctest"
