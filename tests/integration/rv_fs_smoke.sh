#!/usr/bin/env bash
# rv_fs_smoke.sh — PARITY_PLAN.md P2: ext2 mounted on rv64, proven.
#
# One boot of the rv64 kernel with a HOST-FORMATTED ext2 image (the
...
DISK="$BUILD/rv_fs_ext2.img"
LOG="$BUILD/rv_fs.log"

if ! command -v qemu-system-riscv64 >/dev/null 2>&1; then
    echo "[rv-fs] SKIP: qemu-system-riscv64 not installed" >&2
    exit 0
fi
...
if [ -z "$MKFS" ] || [ -z "$DEBUGFS" ]; then
    echo "[rv-fs] SKIP: e2fsprogs not installed (need mkfs.ext2 + debugfs)" >&2
    exit 0
fi
...
if [ "$(wc -c < "$LOG")" -gt 200000 ]; then
    echo "[rv-fs] FAIL: log exceeds the 200KB fuse (runaway output)" >&2
    exit 1
fi
...
assert_grep() {
    ...
    if grep -qa "$pat" "$LOG"; then
        ...
        echo "  [rv-fs] FAIL $desc (pattern: $pat)" >&2
        fail=1
    fi
}
assert_no_grep() {
    local pat="$1" desc="$2"
    if grep -qa "$pat" "$LOG"; then
        echo "  [rv-fs] FAIL $desc (found: $pat)" >&2
        fail=1
    else
        echo "  [rv-fs] OK   $desc"
    fi
...
assert_grep "\[rvfs\] mounted ext2 on blkdev 0"           "arch glue receipt"
assert_grep "\[ext2\] PASS:"                              "ext2 self-test (write/dir/indirect/rename) on rv64"
...
# A fresh disk with a fresh token, attached as virtio-blk-pci: no blk
# on the mmio windows, so vblk falls through to the ECAM walk's bus.
# Same shell, same cat -- the transport is the only variable.
TOKEN2="RVPCI_$$_$(date +%s)"
DISK2="$BUILD/rv_fs_pci_ext2.img"
LOGP="$BUILD/rv_fs_pci.log"
rm -f "$DISK2"
dd if=/dev/zero of="$DISK2" bs=1M count=4 status=none
"$MKFS" -q -b 1024 -I 128 -F "$DISK2" >/dev/null 2>&1
SEED2="$BUILD/rv_fs_pci_seed.$$.txt"
...
if [ "$(wc -c < "$LOGP")" -gt 200000 ]; then
    echo "[rv-fs] FAIL: PCI-lane log exceeds the 200KB fuse" >&2
    exit 1
fi
...
if [ "$fail" -ne 0 ]; then
    echo "[rv-fs] FAILED — log tail:" >&2
    tail -30 "$LOG" >&2
    echo "[rv-fs] PCI-lane log tail:" >&2
    tail -30 "$LOGP" >&2
    exit 1
fi
echo "[rv-fs] all assertions passed (mmio and PCI lanes)"
